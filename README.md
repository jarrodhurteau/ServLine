...
Day 138 — Sprint 13.3 Day 2: Wizard Polish — Apply-to-All Variants + Lightbox Zoom/Drag. "Apply labels to all" button: per-item button (shown when item has variants) pushes variant labels from source item to every other item in the category with the same variant count. Confirm dialog previews labels + target count. Preserves prices — only labels change. New endpoint: POST /api/drafts/<id>/wizard/apply_variant_labels. Solves hallucinated size labels (e.g. fix "14\"-16\"" → "Large" on one pizza, apply to all 27 other pizzas). Lightbox zoom + pan: scroll-to-zoom (1x–8x range) with cursor-centered scaling, click-drag to pan, double-click to toggle 3x zoom/reset, × close button + Escape key, mobile pinch-zoom + touch drag support, hint overlay ("Scroll to zoom · Drag to pan · Double-click to reset"). Position backfill: wizard load auto-assigns positions to items with position=None so ordering matches extraction order.
